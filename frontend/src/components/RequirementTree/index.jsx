...
const RequirementTree = ({ projectId, selectedCategoryId, onSelectCategory }) => {
  ...
  const { data: categories = [] } = useQuery({
    queryKey: ['requirementCategories', projectId],
    queryFn: () => requirementService.getCategories(projectId),
    enabled: !!projectId,
  });

  const createMutation = useMutation({
    mutationFn: requirementService.createCategory,
    onSuccess: () => {
      message.success('目录创建成功');
      queryClient.invalidateQueries({ queryKey: ['requirementCategories', projectId] });
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, data }) => requirementService.updateCategory(id, data),
    onSuccess: () => {
  ...
  const deleteMutation = useMutation({
    mutationFn: requirementService.deleteCategory,
    onSuccess: () => {
  ...
  const handleDelete = (id, name) => {
    confirm({
      title: '确认删除',
      content: `确定删除目录 "${name}" 吗？该目录下的需求将移至未分类。`,
      okText: '删除',
      okType: 'danger',
      cancelText: '取消',
  ...
  return (
    <div className="requirement-tree">
      <div className="tree-header">
        <span className="tree-title">需求目录</span>
        <PlusOutlined
